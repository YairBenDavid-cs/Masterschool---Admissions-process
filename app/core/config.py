"""
Application configuration and FSM blueprint loading utilities.
"""
...
class Settings(BaseSettings):
    """
    Pydantic-Settings class that reads environment variables from a .env file.

    Centralizes all environment-driven configuration for the application,
    including the path to the FSM flow configuration file and the project
    display name.

    Attributes:
        FLOW_CONFIG_PATH (str): Relative path to the JSON flow configuration file.
        PROJECT_NAME (str): Human-readable name of the application.
    """
...
def load_flow_config(settings: Settings) -> FlowConfig:
    ...
    Raises:
        FileNotFoundError: If the configuration file cannot be located at
            the specified path.
...
def get_settings() -> Settings:
    ...
    Using @lru_cache ensures that the Settings object is instantiated
...
def get_flow_config() -> FlowConfig:
    ...
    Returns:
        FlowConfig: The cached and validated FSM flow configuration.
    """
